...
import verifyFireToken from "../Google/verifyFireToken";
...
export default async function SignInUserAppleController(req, res) {
  ...
  try {
    ...
    if (error) {
      res
        .status(400)
        .send({ success: false, emailrror: error.details[0].message });
    }
    ...
    if (isValid == false) {
      return res
        .status(400)
        .send({ successs: false, error: "Please sign up, incorrect token" });
    }
