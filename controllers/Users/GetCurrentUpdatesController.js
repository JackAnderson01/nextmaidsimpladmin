...
export default async function GetCurrentUpdatesController(req, res) {
  ...
  try {
    ...
    if (error) {
      ...
      const lastBooking = await BookingEntity.findOne(
        { userEmail: email, progress: "confirmed" }, // Query for bookings with progress equal to 'confirmed'
